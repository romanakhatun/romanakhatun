    ...
    <section id="skills" className="pt-35">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
        {skillCategories.map((category, index) => (
          <div
            key={index}
            className="group relative p-8 rounded-3xl border border-base-300 bg-base-100/30 backdrop-blur-xl transition-all duration-500 hover:border-primary/50 overflow-hidden hover:-translate-y-1"
          >
            {/* Subtle corner glow */}
            <div
            ...
            <h3 className="text-xl text-neutral font-bold mb-6 uppercase tracking-tight flex items-center gap-3">
            ...
            <div className="flex flex-wrap gap-3">
              {category.skills.map((skill) => (
                <div
                  key={skill}
                  className="px-4 py-2 rounded-xl bg-base-200 border border-base-300 text-base-content/80 text-sm font-medium transition-all duration-300 hover:bg-base-300 hover:text-primary hover:border-primary/30"
                >
                  {skill}
                </div>
              ))}
